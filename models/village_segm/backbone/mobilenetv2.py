...
def _make_divisible(v, divisor, min_value=None):
    """
    This interface is taken from the original tf repo.
...
def fixed_padding(kernel_size, dilation):
    ...
    Return:
    (pad_beg, pad_end, pad_beg, pad_end): (左边填充数， 右边填充数， 上边填充数， 下边填充数)
        四条边的填充大小
    """
    kernel_size_effective = kernel_size + (kernel_size - 1) * (dilation - 1)        # 实际的卷积核大小
...
class InvertedResidual(nn.Module):
    ...
    def __init__(self, inp, oup, stride, dilation, expand_ratio):
        ...
        self.use_res_connect = self.stride == 1 and inp == oup      # 残差边
        ...
        layers.extend([
            ...
            nn.BatchNorm2d(oup),
        ])      # List.extend用于在列表末尾一次性追加另一个序列中的多个值
...
class MobileNetV2(nn.Module):
    def __init__(self, num_classes=1000, output_stride=8, width_mult=1.0, inverted_residual_setting=None, round_nearest=8):
        ...
        num_classes: int
            类别数量
        output_stride: int
            输入图像的空间分辨率和输出特征图的空间分辨率的比值
        width_mult: float
            调整每一层的通道数，new_channels = old_channels*width_mult
        inverted_residual_setting: ndarray
            网络结构，如果为空，则为mobilenet的默认结构，shape=(n, 4)
        round_nearest: int
            通道数调整系数,当通道数不是8的倍数时，使用该系数进行调整
        """
        super(MobileNetV2, self).__init__()
        block = InvertedResidual        # 网络块为深度可分离卷积
        input_channel = 32
        last_channel = 1280
        ...
        # 构建首层
        input_channel = _make_divisible(input_channel * width_mult, round_nearest)  # 检查输入通道数，如果不是8的倍数，则进行调整
