...
from utils.cam import draw_CAM
...
def main(config):
    ...
    # draw_cam
    if config['cam']:
        decode_fn = village_cmap()
        for fileName in tqdm(Path(config['cam_args']['img_path']).iterdir()):
            sv_path = Path('../samples') / fileName.name
            draw_CAM(model, fileName, sv_path, decode_fn, visual_heatmap=False)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', default='../configs/test_villageEP_clss.yaml')
