...
def main(config):
    svname = args.name
    if svname is None:
        svname = 'river_segm-' + config['model']
...
def train(train_loader, model, optimizer):
    ...
    for idx, img_data in enumerate(tqdm(train_loader)):
        image, dem, label = img_data[0].cuda(), img_data[1].cuda(), img_data[2].cuda().float()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', default='configs/train_river.yaml')
