...
def main(config):
    svname = args.name
    if svname is None:
        svname = 'landCover_segm-' + config['model'] + '_' + config['model_args']['backbone']
    ...
    val_loader = DataLoader(val_dataset, config['val_batch_size'], pin_memory=True, drop_last=True)
    ...
    milestone = config['optimizer_args']['milestone']
    ...
    for epoch in range(1, max_epoch + 1):
        ...
        train_score = train(train_loader, model, optimizer, train_dataset.n_classes)
        val_score = validate(val_loader, model, val_dataset.n_classes)
        ...
        classes = ['Unknow', 'Barren', 'Forest', 'Farm', 'Water', 'Rangeland', 'Village']
        # classes = ['Urban', 'Agriculture', 'Range', 'Forest', 'Water', 'Barren', 'Unknown']
...
def train(train_loader, model, optimizer, n_classes):
    ...
    for idx, img_data in enumerate(tqdm(train_loader)):
        image, label = img_data[0].cuda(), img_data[1].cuda().long()
        logits = model(image)

        # loss = F.cross_entropy(logits, label)
        losser = utils.FocalLoss(alpha=1, gamma=5)
