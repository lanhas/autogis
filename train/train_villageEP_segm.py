...
import datasets
import utils
from models import village_segm as models
...
def main(config):
    svname = args.name
    if svname is None:
        svname = 'villageEP_segm-' + config['model'] + '_' + config['model_args']['encoder']
...
def train(train_loader, model, model_name, optimizer, n_classes):
    ...
    for idx, img_data in enumerate(tqdm(train_loader)):
        ...
        aves['tic'].add(np.array([*(iou_cls.values())]))
...
def validate(val_loader, model, model_name, n_classes):
    ...
    for idx, img_data in enumerate(tqdm(val_loader)):
        ...
        aves['vic'].add(np.array([*(iou_cls.values())]))
